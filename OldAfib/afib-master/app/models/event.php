...
class Event extends AppModel
{
    ...
    const REVIEW_INSTRUCTIONS = ' AFIB reviewer instructions.doc';
	
    /** Name of a file with instructions for reviewers */
    const REVIEW_INSTRUCTIONS_PDF = ' AFIB reviewer instructions.pdf';
    ...
    const SCRUB_INSTRUCTIONS = 
      'CNICS AFIB event scrubbing protocol.doc';
	  
    /** Name of a file with instructions for scrubbers */
    const SCRUB_INSTRUCTIONS_PDF = 
      'CNICS AFIB event scrubbing protocol.pdf';
    ...
    static function initialize() {
        ...
        self::$actions = Action::actionArray('Event', array(
            ...
            array('scrub', 'scrubber', 
                  array(self::UPLOADED, self::SCRUBBED), 
...
//            $this->EventDerivedData->addAfterThree($event);
	...
	/* did the reviewers think there was some AF event during this 
	   encounter? */
	$duringThisEncounter1 = $review1['af_foundonly_flag'] ||
            ($review1['afib_flag'] && $review1['afib_encounter_flag']) ||
            ($review1['aflutter_flag'] && $review1['aflutter_encounter_flag']);
	$duringThisEncounter2 = $review2['af_foundonly_flag'] ||
            ($review2['afib_flag'] && $review2['afib_encounter_flag']) ||
            ($review2['aflutter_flag'] && $review2['aflutter_encounter_flag']);


        if (($duringThisEncounter1 == $duringThisEncounter2) &&
	     (!$duringThisEncounter1 || 
	     ($review1['af_secondary_flag'] == $review2['af_secondary_flag'])))
        {  // agreement
            $event['Event']['status'] = self::DONE;
...
//$this->EventDerivedData->catchup();
    ...
    const NO_DECISION = 'Must choose at least one type of AF, or choose "No A fib and no A flutter."';
    const NO_AND_YES = 'Cannot choose one type of AF *and* choose "No AF."';
    const MISSING_ASSOC_DATA = 'Must specify at least one associated condition, or choose "none"';
    const NO_AND_YES_ASSOC = 'Cannot choose an associated condition *and* choose "none"';
    const NO_TOBACCO = 'Must choose a tobacco status';
    const NO_SECONDARY = 'Secondary acute condition cannot be blank';
    const NO_AF_TYPE = 'Must choose an AF type (paroxysmal/persistent/etc.)';
    const NO_AF_TIMING = 'Must specify AF timing (presented in AF/AF started after admission)';
    const MISSING_AFIB_DATA = 'Must choose at least one of "During this encounter/History of A fib"';
    const MISSING_AFLUTTER_DATA = 'Must choose at least one of "During this encounter/History of A flutter"';
    const MISSING_ASSOC_INFECTION_DATA = 'Must specify infection type';
    const MISSING_INFECTION_OTHER = 'Must specify type of other infection';
    const MISSING_ASSOC_THORACIC_DATA = 'Must specify type of thoracic disease';
    const MISSING_THORACIC_OTHER = 'Must specify type of other thoracic disease';
    const MISSING_SECONDARY_OTHER = 'Must specify type of other acute secondary condition';
    const MISSING_ECHO_OTHER = 'Must specify echocardiogram other';
    const EF_NOTPERCENT = 'LV ejection fraction percentage must be number between 0 and 100';
    const EF_PERCENT_AND_TEXT = 'Cannot specify both LV ejection fraction percentage and text';
    const DIM_NOT_NUMBER = 'LA dimension cm must be a number';
    const DIM_CM_AND_TEXT = 'Cannot specify both LA dimension in cm and text';
    const VOLUME_NOT_NUMBER = 'LA volume index must be a number';
...
$this->log("review: " . print_r($data['Review'], true));
	   ...
	   there's an error and they have to redo the form */
        $af_secondary_flag = $data['Review']['af_secondary_flag'];

        if ($af_secondary_flag === '') {
            unset($data['Review']['af_secondary_flag']);
        }

        // most of the form is skipped if 'no af' or afib/aflutter in 'history'
        $noAfNow = $r['no_af_flag'] === '1' || 
                   (!$r['afib_encounter_flag'] &&
                    !$r['aflutter_encounter_flag'] &&
                    !$r['af_foundonly_flag']);

        if (!$r['afib_flag'] && !$r['aflutter_flag'] && 
            !$r['af_foundonly_flag'] && !$r['no_af_flag']) 
        {
            return array(false, self::NO_DECISION);
        } else if ($r['no_af_flag'] === '1' && 
                   ($r['afib_flag'] === '1' || $r['aflutter_flag'] === '1' || 
                    $r['af_foundonly_flag'] === '1')) 
        {
            return array(false, self::NO_AND_YES);
        } else if ($noAfNow) {
	    // the history tags can be set if the corresponding main AF is
	    if (!$r['afib_flag']) {
                $data['Review']['afib_history_flag'] = 0;
	    } 

	    if (!$r['aflutter_flag']) {
                $data['Review']['aflutter_history_flag'] = 0;
	    }

            $data['Review']['afib_encounter_flag'] = 0;
            $data['Review']['aflutter_encounter_flag'] = 0;
            unset($data['Review']['af_timing']);
            $data['Review']['assoc_coronary_flag'] = 0;
            $data['Review']['assoc_mi_flag'] = 0;
            $data['Review']['assoc_hf_flag'] = 0;
            $data['Review']['assoc_vhd_flag'] = 0;
            $data['Review']['assoc_copd_flag'] = 0;
            $data['Review']['assoc_stroke_flag'] = 0;
            $data['Review']['assoc_infection_flag'] = 0;
            $data['Review']['assoc_thoracic_flag'] = 0;
            $data['Review']['assoc_pvd_flag'] = 0;
            $data['Review']['assoc_surgery_flag'] = 0;
            $data['Review']['assoc_none_flag'] = 0;
            $data['Review']['assoc_coronary_angina_flag'] = 0;
            $data['Review']['assoc_coronary_cabg_flag'] = 0;
            $data['Review']['assoc_mi_30_flag'] = 0;
            $data['Review']['assoc_mi_gt30_flag'] = 0;
            $data['Review']['assoc_mi_unknown_flag'] = 0;
            $data['Review']['assoc_hf_exacerbation_flag'] = 0;
            $data['Review']['assoc_vhd_surgery_flag'] = 0;
            $data['Review']['assoc_vhd_disease_flag'] = 0;
            $data['Review']['assoc_copd_exacerbatioin_flag'] = 0;
            $data['Review']['assoc_stroke_current_flag'] = 0;
            $data['Review']['assoc_infection_sepsis_flag'] = 0;
            $data['Review']['assoc_infection_bacteremia_flag'] = 0;
            $data['Review']['assoc_infection_pneumonia_flag'] = 0;
            $data['Review']['assoc_infection_other_flag'] = 0;
            unset($data['Review']['assoc_infection_other']);
            $data['Review']['assoc_thoracic_malignancy_flag'] = 0;
            $data['Review']['assoc_thoracic_mass_flag'] = 0;
            $data['Review']['assoc_thoracic_pericarditis_flag'] = 0;
            $data['Review']['assoc_thoracic_ild_flag'] = 0;
            $data['Review']['assoc_thoracic_ph_flag'] = 0;
            $data['Review']['assoc_thoracic_other_flag'] = 0;
            unset($data['Review']['assoc_thoracic_other']);
            unset($data['Review']['tobacco']);
            $data['Review']['ha_flag'] = 0;
            $data['Review']['ha_intox_flag'] = 0;
            $data['Review']['sub_other_flag'] = 0;
            $data['Review']['sub_other_marijuana_flag'] = 0;
            $data['Review']['sub_other_meth_flag'] = 0;
            $data['Review']['sub_other_meth_intox_flag'] = 0;
            $data['Review']['sub_other_cocaine_flag'] = 0;
            $data['Review']['sub_other_cocaine_intox_flag'] = 0;
            $data['Review']['sub_other_opiate_flag'] = 0;
            $data['Review']['sub_other_opiate_intox_flag'] = 0;
            $data['Review']['sub_other_unspecified_flag'] = 0;
            unset($data['Review']['af_secondary_flag']);
            $data['Review']['secondary_infection_flag'] = 0;
            $data['Review']['secondary_alcohol_flag'] = 0;
            $data['Review']['secondary_thoracic_flag'] = 0;
            $data['Review']['secondary_postop_flag'] = 0;
            $data['Review']['secondary_other_flag'] = 0;
            unset($data['Review']['secondary_other']);
            $data['Review']['echo_flag'] = 0;
            unset($data['Review']['echo_ef_percent']);
            unset($data['Review']['echo_ef_text']);
            $data['Review']['echo_lvseg_flag'] = 0;
            $data['Review']['echo_lvgh_flag'] = 0;
            $data['Review']['echo_lvdd_flag'] = 0;
            $data['Review']['echo_lvh_flag'] = 0;
            $data['Review']['echo_lve_flag'] = 0;
            unset($data['Review']['echo_lae_dimension_cm']);
            unset($data['Review']['echo_lae_dimension_text']);
            unset($data['Review']['echo_lae_volume']);
            $data['Review']['echo_rv_flag'] = 0;
            $data['Review']['echo_elevatedpressure_flag'] = 0;
            unset($data['Review']['echo_valve_disease']);
            $data['Review']['echo_other_flag'] = 0;
            unset($data['Review']['echo_other']);
            unset($data['Review']['af_type']);
            $data['Review']['antic_flag'] = 0;
            $data['Review']['antic_already_flag'] = 0;
            $data['Review']['antic_already_warfarin_flag'] = 0;
            $data['Review']['antic_already_noac_flag'] = 0;
            $data['Review']['antic_already_aspirin_flag'] = 0;
            $data['Review']['antic_already_other_flag'] = 0;
            $data['Review']['antic_already_unknown_flag'] = 0;
            $data['Review']['antic_prescribed_flag'] = 0;
            $data['Review']['antic_prescribed_warfarin_flag'] = 0;
            $data['Review']['antic_prescribed_noac_flag'] = 0;
            $data['Review']['antic_prescribed_aspirin_flag'] = 0;
            $data['Review']['antic_prescribed_other_flag'] = 0;
            $data['Review']['antic_prescribed_unknown_flag'] = 0;

            return array(true);
        } else { // some AF chosen
            if (!$r['assoc_coronary_flag'] && !$r['assoc_mi_flag'] &&
                !$r['assoc_hf_flag'] && !$r['assoc_vhd_flag'] &&
                !$r['assoc_copd_flag'] && !$r['assoc_stroke_flag'] &&
                !$r['assoc_infection_flag'] && !$r['assoc_thoracic_flag'] &&
                !$r['assoc_pvd_flag'] && !$r['assoc_surgery_flag'] &&
                !$r['assoc_none_flag']) 
            {
                return array(false, self::MISSING_ASSOC_DATA);
            } else if ($r['assoc_none_flag'] && 
                ($r['assoc_coronary_flag'] || $r['assoc_mi_flag'] ||
                 $r['assoc_hf_flag'] || $r['assoc_vhd_flag'] ||
                 $r['assoc_copd_flag'] || $r['assoc_stroke_flag'] ||
                 $r['assoc_infection_flag'] || $r['assoc_thoracic_flag'] ||
                 $r['assoc_pvd_flag'] || $r['assoc_surgery_flag']))
            {
                return array(false, self::NO_AND_YES_ASSOC);
            } else if (!$r['tobacco']) {
                return array(false, self::NO_TOBACCO);
            } else if ($af_secondary_flag === '') {
                return array(false, self::NO_SECONDARY);
            } else if (!$r['af_type']) {
                return array(false, self::NO_AF_TYPE);
            }
        }

        $r = $data['Review'];

        if ($r['afib_encounter_flag'] === '1' || 
            $r['aflutter_encounter_flag'] === '1' ||
            $r['af_foundonly_flag'] === '1') 
        {
            if (!$r['af_timing']) {
                return array(false, self::NO_AF_TIMING);
            }
        } else {
            unset($data['Review']['af_timing']);
        }
        ...
        if ($r['afib_flag'] === '1') {
            if (!$r['afib_encounter_flag'] && !$r['afib_history_flag']) {
                return array(false, self::MISSING_AFIB_DATA);
            }
        } else {
            $data['Review']['afib_encounter_flag'] = 0;
            $data['Review']['afib_history_flag'] = 0;
        ...
        if ($r['aflutter_flag'] === '1') {
            if (!$r['aflutter_encounter_flag'] &&
                !$r['aflutter_history_flag']) 
            {
                return array(false, self::MISSING_AFLUTTER_DATA);
            }
        } else {
            $data['Review']['aflutter_encounter_flag'] = 0;
            $data['Review']['aflutter_history_flag'] = 0;
        ...
        if (!$r['assoc_copd_flag']) {
            $data['Review']['assoc_copd_exacerbation_flag'] = 0;
        }

        if (!$r['assoc_stroke_flag']) {
            $data['Review']['assoc_stroke_current_flag'] = 0;
        }

        $r = $data['Review'];

        if ($r['assoc_infection_flag'] === '1') {
            if (!$r['assoc_infection_pneumonia_flag'] && 
                !$r['assoc_infection_sepsis_flag'] && 
                !$r['assoc_infection_bacteremia_flag'] && 
                !$r['assoc_infection_other_flag'])
            {
                return array(false, self::MISSING_ASSOC_INFECTION_DATA);
            }
        } else {
            $data['Review']['assoc_infection_pneumonia_flag'] = 0;
            $data['Review']['assoc_infection_sepsis_flag'] = 0;
        ...
        if ($r['assoc_infection_other_flag'] === '1') {
            if (!$r['assoc_infection_other']) {
        ...
        if ($r['assoc_thoracic_flag'] === '1') {
            if (!$r['assoc_thoracic_malignancy_flag'] && 
                !$r['assoc_thoracic_mass_flag'] && 
                !$r['assoc_thoracic_pericarditis_flag'] && 
                !$r['assoc_thoracic_ild_flag'] && 
                !$r['assoc_thoracic_ph_flag'] && 
                !$r['assoc_thoracic_other_flag'])
            {
                return array(false, self::MISSING_ASSOC_THORACIC_DATA);
            }
        } else {
            $data['Review']['assoc_thoracic_malignancy_flag'] = 0;
            $data['Review']['assoc_thoracic_mass_flag'] = 0;
        ...
        if ($r['assoc_thoracic_other_flag'] === '1') {
            if (!$r['assoc_thoracic_other']) {
                return array(false, self::MISSING_THORACIC_OTHER);
            }
        } else {
            unset($data['Review']['assoc_thoracic_other']);
        }
        ...
        if (!$r['sub_other_meth_flag']) {
            $data['Review']['sub_other_meth_intox_flag'] = 0;
        }

        if (!$r['sub_other_cocaine_flag']) {
            $data['Review']['sub_other_cocaine_intox_flag'] = 0;
        }

        if (!$r['sub_other_opiate_flag']) {
            $data['Review']['sub_other_opiate_intox_flag'] = 0;
        }

        $r = $data['Review'];

        if (!$r['af_secondary_flag']) {
            $data['Review']['secondary_infection_flag'] = 0;
            $data['Review']['secondary_alcohol_flag'] = 0;
            $data['Review']['secondary_thoracic_flag'] = 0;
            $data['Review']['secondary_postop_flag'] = 0;
        ...
        if ($r['secondary_other_flag'] === '1') {
            if (!$r['secondary_other']) {
                return array(false, self::MISSING_SECONDARY_OTHER);
            }
        } else {
            unset($data['Review']['secondary_other']);
        }
        ...
        if ($r['echo_other_flag'] === '1') {
            if (!$r['echo_other']) {
                return array(false, self::MISSING_ECHO_OTHER);
            }
        } else {
            unset($data['Review']['echo_other']);
        }
        ...
        if (!empty($r['echo_ef_percent']) && !empty($r['echo_ef_text'])) {
            return array(false, self::EF_PERCENT_AND_TEXT);
        }

        if (!empty($r['echo_lae_dimension_cm']) && 
            !empty($r['echo_lae_dimension_text'])) 
        {
            return array(false, self::DIM_CM_AND_TEXT);
        }

        if (!empty($r['echo_ef_percent'])) {
            $p = $r['echo_ef_percent'];

            if (!is_numeric($p) || $p > 100 || $p < 0) {
                return array(false, self::EF_NOTPERCENT);
            }
        }

        if (!empty($r['echo_lae_dimension_cm']) && 
            !is_numeric($r['echo_lae_dimension_cm']))
        {
            return array(false, self::DIM_NOT_NUMBER);
        }

        if (!empty($r['echo_lae_volume']) && 
            !is_numeric($r['echo_lae_volume']))
        {
            return array(false, self::VOLUME_NOT_NUMBER);
        }
