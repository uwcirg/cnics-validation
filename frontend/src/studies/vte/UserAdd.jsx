...
function VTEUserAdd({ auth }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    try {
      const res = await fetch(`${apiUrl}/api/users?study=vte`, {
        method: 'POST',
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button type="submit">Add VTE User</button>
      </form>
      {status === 'saved' && <p>VTE user saved.</p>}
      {status === 'error' && <p>Failed to save VTE user.</p>}
    </div>
  )
}

export default VTEUserAdd
