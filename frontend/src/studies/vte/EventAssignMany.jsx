...
function VTEEventAssignMany() {
  ...
  useEffect(() => {
    // Load events needing assignment
    fetch(`${apiUrl}/api/events/by_status/screened?study=vte&limit=50&offset=0`, { credentials: 'include' })
    ...
    fetch(`${apiUrl}/api/tables/users?limit=2000`, { credentials: 'include' })
      .then((r) => r.ok ? r.json() : Promise.reject(r))
      .then((j) => {
        const all = (j.data || []).filter((u) => u.reviewer_flag)
        setReviewers(all)
      })
      .catch(() => setReviewers([]))
  }, [apiUrl])
  ...
  const handleAssign = async (e) => {
    ...
    if (!ids.length || !reviewer1Id || !reviewer2Id) return
    setStatus('saving')
    try {
      const res = await fetch(`${apiUrl}/api/events/assign_many?study=vte`, {
        ...
        body: JSON.stringify({ 
          event_ids: ids, 
          reviewer1_id: Number(reviewer1Id), 
          reviewer2_id: Number(reviewer2Id)
        }),
      })
      if (res.ok) {
        setStatus('saved')
        showToast('Assigned reviewers to selected events.', 'success')
        // Clear selection
        setSelected({})
  ...
  return (
    <div>
      <h1>Assign VTE Charts to Reviewers</h1>
      ...
      <h2 style={{ marginTop: '16px' }}>Step 2: Choose Reviewers</h2>
      <div className="indent1">
        <div style={{ marginBottom: '12px' }}>
          <label>First Reviewer: </label>
          <select value={reviewer1Id} onChange={(e) => setReviewer1Id(e.target.value)}>
      ...
      <div style={{ marginTop: '12px' }}>
        <button onClick={handleAssign} disabled={!reviewer1Id || !reviewer2Id}>
          Assign Reviewers
        </button>
      </div>
...
export default VTEEventAssignMany
