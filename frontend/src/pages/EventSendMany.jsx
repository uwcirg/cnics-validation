...
function EventSendMany() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const listRes = await fetch(`${apiUrl}/api/events/by_status/assigned?limit=100&offset=0`, { credentials: 'include' })
  ...
  const handleSend = async (e) => {
    ...
    setStatus('saving')
    try {
      const res = await fetch(`${apiUrl}/api/events/send_many`, {
        method: 'POST',
      ...
      if (res.ok) {
        setStatus('saved')
        showToast('Charts sent to reviewers for selected events.', 'success')
