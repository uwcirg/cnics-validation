...
class Event extends AppModel
{
    ...
    const REVIEW_INSTRUCTIONS = ' HF reviewer instructions.doc';
	
    /** Name of a file with instructions for reviewers */
    const REVIEW_INSTRUCTIONS_PDF = ' HF reviewer instructions.pdf';
    ...
    const SCRUB_INSTRUCTIONS = 
      'CNICS HF event scrubbing protocol.doc';
	  
    /** Name of a file with instructions for scrubbers */
    const SCRUB_INSTRUCTIONS_PDF = 
      'CNICS HF event scrubbing protocol.pdf';
	...
	/* did the reviewers agree that there was/was not heart failure? */
	$hftype1 = $review1['hf_type'];
	$hftype2 = $review2['hf_type'];

        if (($hftype1 == Review::NOT_HF && $hftype2 == Review::NOT_HF) ||
		($hftype1 != Review::NOT_HF && $hftype2 != Review::NOT_HF))
        {  // agreement
            $event['Event']['status'] = self::DONE;
...
$this->EventDerivedData->catchup();
    ...
    const NO_SIGNS = 'Must answer the signs or symptoms question';
    const NO_DIAGNOSIS = 'Must answer the diagnosis question';
    const NO_PRESCRIBED = 'Must answer whether the person was prescribed medication';
    const BAD_DATE = 'Bad date: estimated date of onset';

    const NO_CONGESTION = 'Must answer volume overload/congestion question';
    const NO_ECHO = 'Must specify if echocardiogram was performed';
    const NO_HFTYPE = 'Must specify the type of HF';
    const MISSING_ECHO_DATA = 'Must answer the echocardiogram questions';
    const BAD_LVEF = 'Bad LVEF value';

    const NO_CLASSIFICATION = 'Must specify the HF classification';
    const NO_ETIOLOGY = 'Must specify the likely etiology';
    const NO_PRESENTATION = 'Must answer the presentation question';
    const NO_MOREINFO = 'Must answer whether more information is needed';
    const NO_NI_DATA = 'Must specify non-ischemic sub-category/categories';
    const NO_NI_OTHER = 'Must specify the other non-ischemic sub-category';
    const NO_NEED_MORE = 'Must specify what additional information is needed for assessment';
...
$this->log("review: " . print_r($data['Review'], true));
	   ...
	   there's an error and they have to redo the form */
        $signs_flag = $data['Review']['signs_flag'];
        $diagnose_flag = $data['Review']['diagnose_flag'];
        $prescribed_flag = $data['Review']['prescribed_flag'];
        $echo_flag = $data['Review']['echo_flag'];
        $low_lvef_flag = $data['Review']['low_lvef_flag'];
        $wma_flag = $data['Review']['wma_flag'];
        $dd_flag = $data['Review']['dd_flag'];
        $msvd_flag = $data['Review']['msvd_flag'];
        $more_info_flag = $data['Review']['more_info_flag'];

        if ($signs_flag === '') {
            unset($data['Review']['signs_flag']);
        }

        if ($diagnose_flag === '') {
            unset($data['Review']['diagnose_flag']);
        }

        if ($prescribed_flag === '') {
            unset($data['Review']['prescribed_flag']);
        }

        if ($echo_flag === '') {
            unset($data['Review']['echo_flag']);
        }

        if ($low_lvef_flag === '') {
            unset($data['Review']['low_lvef_flag']);
        }

        if ($wma_flag === '') {
            unset($data['Review']['wma_flag']);
        }

        if ($dd_flag === '') {
            unset($data['Review']['dd_flag']);
        }

        if ($msvd_flag === '') {
            unset($data['Review']['msvd_flag']);
        }

        if ($more_info_flag === '') {
            unset($data['Review']['more_info_flag']);
        }

	$onset_date = $data['Review']['onset_date'];

        if ($signs_flag === '') {
            return array(false, self::NO_SIGNS);
        }

        if ($diagnose_flag === '') {
            return array(false, self::NO_DIAGNOSIS);
        }

        if ($prescribed_flag === '') {
            return array(false, self::NO_PRESCRIBED);
        }

        // most of the form is skipped if the first 3 flags are not yes
        $noHf = (!$r['signs_flag'] ||
                    !$r['diagnose_flag'] ||
                    !$r['prescribed_flag']);

        if ($noHf) {
            unset($data['Review']['onset_date']);
            unset($data['Review']['congestion']);
            $data['Review']['echo_flag'] = 0;
            unset($data['Review']['lvef']);
            $data['Review']['low_lvef_flag'] = 0;
            $data['Review']['wma_flag'] = 0;
            $data['Review']['lvcd_flag'] = 0;
            $data['Review']['dd_flag'] = 0;
            $data['Review']['msvd_flag'] = 0;
            $data['Review']['hf_type'] = Review::NOT_HF; // per spec
            unset($data['Review']['classification']);
            $data['Review']['ischemic_flag'] = 0;
            $data['Review']['nonischecmid_flag'] = 0;
            $data['Review']['ni_valvular_flag'] = 0;
            $data['Review']['ni_infiltrative_flag'] = 0;
            $data['Review']['ni_im_flag'] = 0;
            $data['Review']['ni_obstructive_flag'] = 0;
            $data['Review']['ni_recreational_flag'] = 0;
            $data['Review']['ni_prescription_flag'] = 0;
            $data['Review']['ni_hypertensive_flag'] = 0;
            $data['Review']['ni_renal_flag'] = 0;
            $data['Review']['ni_cardiomyopathy_flag'] = 0;
            $data['Review']['ni_covid_flag'] = 0;
            $data['Review']['ni_sepsis_flag'] = 0;
            $data['Review']['ni_metabolic_flag'] = 0;
            $data['Review']['ni_pd_flag'] = 0;
            $data['Review']['ni_hypertrophic_flag'] = 0;
            $data['Review']['ni_other_flag'] = 0;
            unset($data['Review']['ni_other']);
            $data['Review']['unknown_flag'] = 0;
            unset($data['Review']['presentation']);
            $data['Review']['more_info_flag'] = 0;
            unset($data['Review']['need_more']);
            return array(true);
        } else { // some HF indication
	    if (empty($onset_date)) {
                return array(false, self::BAD_DATE);
            }

            if (!$r['congestion']) {
                return array(false, self::NO_CONGESTION);
            }

            if ($r['echo_flag'] === '') {
                return array(false, self::NO_ECHO);
            }

	    if ($r['echo_flag'] == 1) {
                if (!$r['lvef'] || $r['low_lvef_flag'] === '' ||
                    !$r['wma_flag'] === '' ||
		    !$r['dd_flag'] === '' || $r['msvd_flag'] === '')
		{
                    return array(false, self::MISSING_ECHO_DATA);
		}

		$lvef = $r['lvef'];

		if ($lvef != Review::NOT_REPORTED && 
			(intval($lvef) > Review::MAX_LVEF ||
			 intval($lvef) < Review::MIN_LVEF))
		{
                    return array(false, self::BAD_LVEF);
		}
	    }

            $data['Review']['hf_type'] = Review::DEF_OR_PROB_HF; // per spec

            if (!$r['classification']) {
                return array(false, self::NO_CLASSIFICATION);
            }

	    if (!$r['ischemic_flag'] && !$r['nonischemic_flag'] &&
	    	!$r['unknown_flag']) 
	    {
                return array(false, self::NO_ETIOLOGY);
            }

            if (!$r['presentation']) {
                return array(false, self::NO_PRESENTATION);
            }

            if ($r['more_info_flag'] === '') {
                return array(false, self::NO_MOREINFO);
            }

	    if ($r['nonischemic_flag']) {
	        if (!$r['ni_valvular_flag'] &&
		    !$r['ni_infiltrative_flag'] &&
		    !$r['ni_im_flag'] &&
		    !$r['ni_obstructive_flag'] &&
		    !$r['ni_recreational_flag'] &&
		    !$r['ni_prescription_flag'] &&
		    !$r['ni_hypertensive_flag'] &&
		    !$r['ni_renal_flag'] &&
		    !$r['ni_cardiomyopathy_flag'] &&
		    !$r['ni_covid_flag'] &&
		    !$r['ni_sepsis_flag'] &&
		    !$r['ni_metabolic_flag'] &&
		    !$r['ni_pd_flag'] &&
		    !$r['ni_hypertrophic_flag'] &&
		    !$r['ni_other_flag']) 
		{
                    return array(false, self::NO_NI_DATA);
		}

		if ($r['ni_other_flag'] && empty($r['ni_other'])) {
                    return array(false, self::NO_NI_OTHER);
		}
            } else {
                $data['Review']['ni_valvular_flag'] = 0;
                $data['Review']['ni_infiltrative_flag'] = 0;
	    ...
	    if ($r['more_info_flag'] && empty($r['need_more'])) {
                return array(false, self::NO_NEED_MORE);
            }
        }
