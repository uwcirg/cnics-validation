...
class EventsController extends CodedItemsController
{
    ...
    /**
     * Get the 2nd part of prefix for instructions (based on the URL)
     */
    private function getInstructionPrefix2() {
        return (strpos(Router::url('/', true), 'mci') != true) ? 
            'VTE' : 'MI';
    }
    ...
    function index() {
        ...
        $this->set('prefix2', $this->getInstructionPrefix2());
    }
    ...
    /**
     * Prescrub charts for an events
     * @param id Id of the event
     */
    function prescrub($id = null) {
        if (!empty($this->data)) {
            $id = intval($this->data['Event']['id']);
        } else {
            $id = intval($id);
        ...
        if (!empty($this->data) && !empty($this->data['Event'])) {
            $status = $this->Event->prescrub($event, $this->data['Event'],
    ...
    private function review($reviewerNumber, $eventId) {
            ...
            if (!$result[0]) {
                ...
                $this->Review->save($this->data, array('fieldList' => 
                    array('event_id', 'reviewer_id', 
                          ...
                          'pe_lobar_flag', 
                          'pe_segmental_flag',
                          'pe_subsegmental_flag', 
                          'pe_unknown_flag', 
                          'dvt_ue_flag', 
                          'dvt_le_flag',
                          'dvt_le_proximal_flag', 
                          'dvt_le_distal_flag',
                          'dvt_le_unknown_flag', 
                          'dvt_other_flag',
                          'dvt_other_neck_flag', 
                          'dvt_other_neck_jugular_flag',
                          'dvt_other_neck_subclavian_flag',
                          'dvt_other_neck_brach_flag',
                          'dvt_other_neck_unknown_flag',
                          'dvt_other_vc_flag',
                          'dvt_other_ap_flag',
                          'dvt_other_ap_renal_flag',
                          'dvt_other_ap_hepatic_flag',
                          'dvt_other_ap_pelvic_flag',
                          'dvt_other_ap_iliac_flag',
                          'dvt_other_ap_unknown_flag',
                          'dvt_other_ps_flag',
                          'dvt_other_ps_hepatic_flag',
                          'dvt_other_ps_splenic_flag',
                          'dvt_other_ps_mesenteric_flag',
                          'dvt_other_ps_unknown_flag',
                          'dvt_other_ic_flag',
                          'dvt_other_ic_sst_flag',
                          'dvt_other_ic_tst_flag',
                          'dvt_other_ic_rvt_flag',
                          'dvt_other_ic_unknown_flag',
                          'dvt_other_other_flag',
                          'dvt_other',
                          'dvt_other_unknown_flag',
                          'dvt_unknown_flag', 
                          'cc_malignancy_flag',
                          'cc_chemo_flag',
                          'cc_heartfailure_flag',
                          'cc_ns_flag',
                          'cc_dialysis_flag',
                          'cc_hosp_flag',
                          'cc_mt_flag',
                          ...
                          'cc_surgery_flag',
                          'cc_infection_flag',
                          'cc_infection_pneumonia_flag',
                          'cc_infection_sepsis_flag',
                          'cc_infection_uti_flag',
                          'cc_infection_endocarditis_flag',
                          'cc_infection_osteomyelitis_flag',
                          'cc_infection_meningitis_flag',
                          'cc_infection_cellulitis_flag',
                          'cc_infection_covid_flag',
                          'cc_infection_other_flag',
                          'cc_infection_other',
                          'cc_transfusion_flag',
                          'cc_inherited_flag',
                          'cc_ivdrug_flag',
                          ...
                          'vtehistory_pe_flag',
                          'vtehistory_dvt_flag',
                          'vtehistory_unknowntype_flag',
                          'vtehistory_none_flag',
                          'vtehistory_unknown_flag',
                          'family_history',
                          'management_info',
                          'management_at',
                          'management_hosp',
                          'management_vcf',
                          'management_tt',
                          'management_thrombectomy',
                          'management_managedas')));
        ...
        $this->set('dps', $this->Review->getDps());
        $this->set('types', $this->Review->getTypes());
        $this->set('catSubtypes', $this->Review->getCatSubtypes());
        $this->set('ivDrugUses', $this->Review->getIVDrugUses());
        $this->set('smokingUses', $this->Review->getSmokingUses());
        $this->set('multipleChoices', $this->Review->getMultipleChoices());
        $this->set('managementAts', $this->Review->getManagementAts());
        $this->set('managementManagedAs', 
                   $this->Review->getManagementManagedAs());
    ...
    function viewAll() {
        ...
        $this->set('toBePrescrubbed', $this->Event->getAll(Event::UPLOADED));
        $this->set('toBeScrubbed', $this->Event->getAll(Event::PRESCRUBBED));
        ...
        $this->set('psrejected', $this->Event->getAll(Event::PSREJECTED));
	...
	$dps = $this->Review->getDps();
	$dps[EventDerivedData::NO_CONSENSUS] =
	    EventDerivedData::NO_CONSENSUS;
	$types = $this->Review->getTypes();
	$types[EventDerivedData::NO_CONSENSUS] =
	    EventDerivedData::NO_CONSENSUS;
	$catSubtypes = $this->Review->getCatSubtypes();
	$catSubtypes[EventDerivedData::NO_CONSENSUS] =
	    EventDerivedData::NO_CONSENSUS;
        $this->set('dps', $dps);
        $this->set('types', $types);
        $this->set('catSubtypes', $catSubtypes);
               ...
               with if does not yield the same logic */
                    ...
                    $this->Event->toBeScreened($event, $this->authUser)) 
                {
                    ...
                    $downloadPrefix = Event::SCRUBBED_PREFIX;
                     ...
                     || $this->Event->canBePrescrubbed($event, $this->authUser))
                {
