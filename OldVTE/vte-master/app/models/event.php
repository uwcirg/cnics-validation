...
class Event extends AppModel
{
    ...
    var $belongsTo = array('Patient', 
                           ...
                           'Prescrubber' => array('className' => 'User',
                                              'foreignKey' => 'prescrubber_id'),
    ...
    //bogus change to force a git merge
    ...
    /** Value for status field that indicates event has been prescrubbed */
    const PRESCRUBBED = 'prescrubbed';
    ...
    /** Value for status field that indicates event did not pass prescrub */
    const PSREJECTED = 'psrejected';
    ...
    const REVIEW_INSTRUCTIONS = ' VTE reviewer instructions.doc';
	
    /** Name of a file with instructions for reviewers */
    const REVIEW_INSTRUCTIONS_PDF = ' VTE reviewer instructions.pdf';
    ...
    const SCRUB_INSTRUCTIONS = 
      'CNICS VTE event scrubbing protocol.doc';
	  
    /** Name of a file with instructions for scrubbers */
    const SCRUB_INSTRUCTIONS_PDF = 
      'CNICS VTE event scrubbing protocol.pdf';
    ...
    private static $statusNames = array(
        self::CREATED, self::UPLOADED, self::PRESCRUBBED, 
        self::SCRUBBED, self::SCREENED,
        self::ASSIGNED,
        self::SENT, self::REVIEWER1_DONE, self::REVIEWER2_DONE,
        self::THIRD_REVIEW_NEEDED, self::THIRD_REVIEW_ASSIGNED,
        self::DONE, self::PSREJECTED, self::REJECTED, 
        self::NO_PACKET_AVAILABLE);
    ...
    static function initialize() {
        ...
        self::$actions = Action::actionArray('Event', array(
            ...
            array('prescrub', 'prescrubber', array(self::UPLOADED), 
                  self::$isAdmin, null, true),
            array('scrub', 'scrubber', 
                  array(self::PRESCRUBBED, self::SCRUBBED), 
    ...
    function updateAfterReview($reviewNumber, $event, $reviewer) {
        ...
        if ($review1['pe_flag'] == $review2['pe_flag'] && 
            $review1['dvt_flag'] == $review2['dvt_flag'] && 
            $review1['cat_flag'] == $review2['cat_flag'] && 
            $review1['no_vte_flag'] == $review2['no_vte_flag'])
        {  // agreement
            $event['Event']['status'] = self::DONE;
...
//$this->EventDerivedData->catchup();
            ...
            (!empty($file['tmp_name']) && $file['tmp_name'] != 'none'))
        {
            if (!defined('CAKEPHP_UNIT_TEST_EXECUTION')) {
                ...
                return (strpos($file['tmp_name'], '/tmp') === 0) ||
                       (strpos($file['tmp_name'], '/srv/www/cnics.cirg.washington.edu/htdocs/mci-dev/app/tests/cases/controllers') === 0);
            }
        } else {
            return false;
        }
    ...
    const NO_DECISION = 'Must choose at least one type of VTE, or choose "No VTE."';
    const NO_AND_YES = 'Cannot choose one type of VTE *and* choose "No VTE."';
    const MISSING_CC_DATA = 'Must specify at least one contributing condition';
    const NO_AND_YES_CC = 'Cannot choose a contributing condition *and* choose "None"';
    const NO_SMOKING_USE = 'Must choose a smoking status';
    const NO_FAMILY_HISTORY = 'Must choose a family history';
    const NO_MANAGEMENT_INFO = 'Must specify if management information is available';
    const NO_VTE_HISTORY = 'Must specify history of prior VTE';
    const NO_AND_YES_VTE_HISTORY = 'Cannot choose a VTE history type *and* choose "None" or "Unknown"';
    const MISSING_PE_DATA = 'Must specify type and location of PE';
    const MISSING_DVT_DATA = 'Must specify type and location of DVT';
    const MISSING_CAT_DATA = 'Must specify type and location of Catheter-associated thrombosis';
    const MISSING_LE_DATA = 'Must specify location of DVT LE';
    const MISSING_DVT_OTHER_DATA = 'Must specify DVT Other location';
    const MISSING_DVT_NECK_DATA = 'Must specify neck/chest location';
    const MISSING_DVT_AP_DATA = 'Must specify Abdomen/pelvis location';
    const MISSING_DVT_PS_DATA = 'Must specify Portal system location';
    const MISSING_DVT_IC_DATA = 'Must specify Intracranial location';
    const MISSING_DVT_OTHER = 'Must specify Other location';
    const MISSING_CC_INFECTION_DATA = 'Must specify infection type';
    const MISSING_INFECTION_OTHER = 'Must specify type of Other infection';
    const MISSING_IVDRUG_USE = 'Must specify time of IV drug use';
    const MISSING_CC_OTHER = 'Must specify other contributing condition';
    const MISSING_MANAGEMENT_DATA = 'Must complete Management information';
...
//$this->log("review: " . print_r($data['Review'], true));

        if (!$r['pe_flag'] && !$r['dvt_flag'] && !$r['cat_flag'] && 
            !$r['no_vte_flag']) 
        {
            return array(false, self::NO_DECISION);
        } else if ($r['no_vte_flag'] === '1' && 
                   ($r['pe_flag'] === '1' || $r['dvt_flag'] === '1' || 
                    $r['cat_flag'] === '1')) 
        {
            return array(false, self::NO_AND_YES);
        } else if ($r['no_vte_flag'] === '1') {
            $data['Review']['cc_malignancy_flag'] = 0;
            $data['Review']['cc_chemo_flag'] = 0;
            $data['Review']['cc_heartfailure_flag'] = 0;
            $data['Review']['cc_ns_flag'] = 0;
            $data['Review']['cc_dialysis_flag'] = 0;
            $data['Review']['cc_hosp_flag'] = 0;
            $data['Review']['cc_mt_flag'] = 0;
            $data['Review']['cc_immob_flag'] = 0;
            $data['Review']['cc_longride_flag'] = 0;
            $data['Review']['cc_surgery_flag'] = 0;
            $data['Review']['cc_infection_flag'] = 0;
            $data['Review']['cc_transfusion_flag'] = 0;
            $data['Review']['cc_inherited_flag'] = 0;
            $data['Review']['cc_ivdrug_flag'] = 0;
            $data['Review']['cc_copd_flag'] = 0;
            $data['Review']['cc_ph_flag'] = 0;
            $data['Review']['cc_steroid_flag'] = 0;
            $data['Review']['cc_pregnancy_flag'] = 0;
            $data['Review']['cc_other_flag'] = 0;
            $data['Review']['cc_unknown_flag'] = 0;
            $data['Review']['cc_none_flag'] = 0;
            unset($data['Review']['smoking_use']);
            $data['Review']['vtehistory_pe_flag'] = 0;
            $data['Review']['vtehistory_dvt_flag'] = 0;
            $data['Review']['vtehistory_unknowntype_flag'] = 0;
            $data['Review']['vtehistory_none_flag'] = 0;
            $data['Review']['vtehistory_unknown_flag'] = 0;
            unset($data['Review']['family_history']);
            unset($data['Review']['management_info']);
        } else { // at least one of PE/DVT/CAT set
            if (!$r['cc_malignancy_flag'] && !$r['cc_chemo_flag'] &&
                !$r['cc_heartfailure_flag'] && !$r['cc_ns_flag'] &&
                !$r['cc_dialysis_flag'] && !$r['cc_hosp_flag'] &&
                !$r['cc_mt_flag'] && !$r['cc_immob_flag'] &&
                !$r['cc_longride_flag'] && !$r['cc_surgery_flag'] &&
                !$r['cc_infection_flag'] && !$r['cc_transfusion_flag'] &&
                !$r['cc_inherited_flag'] && !$r['cc_ivdrug_flag'] &&
                !$r['cc_copd_flag'] && !$r['cc_ph_flag'] &&
                !$r['cc_steroid_flag'] && !$r['cc_pregnancy_flag'] &&
                !$r['cc_other_flag'] && 
                !$r['cc_unknown_flag'] && !$r['cc_none_flag']) 
            {
                return array(false, self::MISSING_CC_DATA);
            } else if ($r['cc_none_flag'] && 
                ($r['cc_malignancy_flag'] || $r['cc_chemo_flag'] ||
                 $r['cc_heartfailure_flag'] || $r['cc_ns_flag'] ||
                 $r['cc_dialysis_flag'] || $r['cc_hosp_flag'] ||
                 $r['cc_mt_flag'] || $r['cc_immob_flag'] ||
                 $r['cc_longride_flag'] || $r['cc_surgery_flag'] ||
                 ...
                 $r['cc_copd_flag'] || $r['cc_ph_flag'] ||
                 $r['cc_steroid_flag'] || $r['cc_pregnancy_flag'] ||
                 $r['cc_other_flag'] || $r['cc_unknown_flag']) )
            {
                return array(false, self::NO_AND_YES_CC);
            } else if (!$r['smoking_use']) {
                return array(false, self::NO_SMOKING_USE);
            } else if (!$r['family_history']) {
                return array(false, self::NO_FAMILY_HISTORY);
            } else if (!isset($r['management_info'])) {
                return array(false, self::NO_MANAGEMENT_INFO);
            } else if (!$r['vtehistory_pe_flag'] && 
                       !$r['vtehistory_dvt_flag'] &&
                       !$r['vtehistory_unknowntype_flag'] &&
                       !$r['vtehistory_none_flag'] &&
                       !$r['vtehistory_unknown_flag']) 
            {
                return array(false, self::NO_VTE_HISTORY);
            } else if ((($r['vtehistory_none_flag']) && 
                        ($r['vtehistory_pe_flag'] || 
                         $r['vtehistory_dvt_flag'] ||
                         $r['vtehistory_unknowntype_flag'] ||
                         $r['vtehistory_unknown_flag'])) ||
                       (($r['vtehistory_unknown_flag']) && 
                        ($r['vtehistory_pe_flag'] || 
                         ...
                         $r['vtehistory_none_flag'])))
            {
                return array(false, self::NO_AND_YES_VTE_HISTORY);
            }
        }
        ...
        if ($r['pe_flag'] === '1') {
            if (!$r['pe_dp'] || !$r['pe_type'] || 
                (!$r['pe_main_flag'] && !$r['pe_lobar_flag'] &&
                 !$r['pe_segmental_flag'] && !$r['pe_subsegmental_flag'] && 
                 !$r['pe_unknown_flag']))
            {
                return array(false, self::MISSING_PE_DATA);
            }
        } else {
            unset($data['Review']['pe_dp']);
            unset($data['Review']['pe_type']);
            $data['Review']['pe_main_flag'] = 0;
            $data['Review']['pe_lobar_flag'] = 0;
            $data['Review']['pe_segmental_flag'] = 0;
            $data['Review']['pe_subsegmental_flag'] = 0;
            $data['Review']['pe_unknown_flag'] = 0;
        }

        $r = $data['Review'];

        if ($r['dvt_flag'] === '1') {
            if (!$r['dvt_dp'] || !$r['dvt_type'] || 
                (!$r['dvt_ue_flag'] && !$r['dvt_le_flag'] &&
                 !$r['dvt_other_flag'] && !$r['dvt_unknown_flag']))
            {
                return array(false, self::MISSING_DVT_DATA);
            }
        } else {
            unset($data['Review']['dvt_dp']);
            unset($data['Review']['dvt_type']);
        }

        if ($r['cat_flag'] === '1') {
            if (!$r['cat_subtype'] || !$r['cat_dp'] || !$r['cat_type'] || 
                (!$r['dvt_ue_flag'] && !$r['dvt_le_flag'] &&
                 !$r['dvt_other_flag'] && !$r['dvt_unknown_flag']))
            {
                return array(false, self::MISSING_CAT_DATA);
            }
        } else {
            unset($data['Review']['cat_subtype']);
            unset($data['Review']['cat_dp']);
            unset($data['Review']['cat_type']);
        }

        if (!$r['dvt_flag'] && !$r['cat_flag']) {
            $data['Review']['dvt_ue_flag'] = 0;
            $data['Review']['dvt_le_flag'] = 0;
            $data['Review']['dvt_other_flag'] = 0;
            $data['Review']['dvt_unknown_flag'] = 0;
        }

        $r = $data['Review'];

        if ($r['dvt_le_flag'] === '1') {
            if (!$r['dvt_le_proximal_flag'] && !$r['dvt_le_distal_flag'] && 
                !$r['dvt_le_unknown_flag'])
            {
                return array(false, self::MISSING_LE_DATA);
            }
        } else {
            $data['Review']['dvt_le_proximal_flag'] = 0;
            $data['Review']['dvt_le_distal_flag'] = 0;
            $data['Review']['dvt_le_unknown_flag'] = 0;
        }

        $r = $data['Review'];

        if ($r['dvt_other_flag'] === '1') {
            if (!$r['dvt_other_neck_flag'] && !$r['dvt_other_vc_flag'] && 
                !$r['dvt_other_ap_flag'] && !$r['dvt_other_ps_flag'] &&
                !$r['dvt_other_ic_flag'] && !$r['dvt_other_other_flag'] &&
                !$r['dvt_other_unknown_flag'])
            {
                return array(false, self::MISSING_DVT_OTHER_DATA);
            }
        } else {
            $data['Review']['dvt_other_neck_flag'] = 0;
            $data['Review']['dvt_other_vc_flag'] = 0;
            $data['Review']['dvt_other_ap_flag'] = 0;
            $data['Review']['dvt_other_ps_flag'] = 0;
            $data['Review']['dvt_other_ic_flag'] = 0;
            $data['Review']['dvt_other_other_flag'] = 0;
            $data['Review']['dvt_other_unknown_flag'] = 0;
        }

        $r = $data['Review'];

        if ($r['dvt_other_neck_flag'] === '1') {
            if (!$r['dvt_other_neck_jugular_flag'] && 
                !$r['dvt_other_neck_subclavian_flag'] && 
                !$r['dvt_other_neck_brach_flag'] && 
                !$r['dvt_other_neck_unknown_flag'])
            {
                return array(false, self::MISSING_DVT_NECK_DATA);
            }
        } else {
            $data['Review']['dvt_other_neck_jugular_flag'] = 0;
            $data['Review']['dvt_other_neck_subclavian_flag'] = 0;
            $data['Review']['dvt_other_neck_brach_flag'] = 0;
            $data['Review']['dvt_other_neck_unknown_flag'] = 0;
        }

        $r = $data['Review'];

        if ($r['dvt_other_ap_flag'] === '1') {
            if (!$r['dvt_other_ap_renal_flag'] && 
                !$r['dvt_other_ap_hepatic_flag'] && 
                !$r['dvt_other_ap_pelvic_flag'] && 
                !$r['dvt_other_ap_iliac_flag'] && 
                !$r['dvt_other_ap_unknown_flag'])
            {
                return array(false, self::MISSING_DVT_AP_DATA);
            }
        } else {
            $data['Review']['dvt_other_ap_renal_flag'] = 0;
            $data['Review']['dvt_other_ap_hepatic_flag'] = 0;
            $data['Review']['dvt_other_ap_pelvic_flag'] = 0;
            $data['Review']['dvt_other_ap_iliac_flag'] = 0;
            $data['Review']['dvt_other_ap_unknown_flag'] = 0;
        }

        $r = $data['Review'];

        if ($r['dvt_other_ps_flag'] === '1') {
            if (!$r['dvt_other_ps_hepatic_flag'] && 
                !$r['dvt_other_ps_splenic_flag'] && 
                !$r['dvt_other_ps_mesenteric_flag'] && 
                !$r['dvt_other_ps_unknown_flag'])
            {
                return array(false, self::MISSING_DVT_PS_DATA);
            }
        } else {
            $data['Review']['dvt_other_ps_hepatic_flag'] = 0;
            $data['Review']['dvt_other_ps_splenic_flag'] = 0;
            $data['Review']['dvt_other_ps_mesenteric_flag'] = 0;
            $data['Review']['dvt_other_ps_unknown_flag'] = 0;
        }

        $r = $data['Review'];

        if ($r['dvt_other_ic_flag'] === '1') {
            if (!$r['dvt_other_ic_sst_flag'] && 
                !$r['dvt_other_ic_tst_flag'] && 
                !$r['dvt_other_ic_rvt_flag'] && 
                !$r['dvt_other_ic_unknown_flag'])
            {
                return array(false, self::MISSING_DVT_IC_DATA);
            }
        } else {
            $data['Review']['dvt_other_ic_sst_flag'] = 0;
            $data['Review']['dvt_other_ic_tst_flag'] = 0;
            $data['Review']['dvt_other_ic_rvt_flag'] = 0;
            $data['Review']['dvt_other_ic_unknown_flag'] = 0;
        }

        $r = $data['Review'];

        if ($r['dvt_other_other_flag'] === '1') {
            if (!$r['dvt_other']) {
                return array(false, self::MISSING_DVT_OTHER);
            }
        } else {
            unset($data['Review']['dvt_other']);
        }

        $r = $data['Review'];

        if ($r['cc_infection_flag'] === '1') {
            if (!$r['cc_infection_pneumonia_flag'] && 
                !$r['cc_infection_sepsis_flag'] && 
                !$r['cc_infection_uti_flag'] && 
                !$r['cc_infection_endocarditis_flag'] && 
                !$r['cc_infection_osteomyelitis_flag'] && 
                !$r['cc_infection_meningitis_flag'] && 
                !$r['cc_infection_cellulitis_flag'] && 
                !$r['cc_infection_covid_flag'] && 
                !$r['cc_infection_other_flag'])
            {
                return array(false, self::MISSING_CC_INFECTION_DATA);
            }
        } else {
            $data['Review']['cc_infection_pneumonia_flag'] = 0;
            $data['Review']['cc_infection_sepsis_flag'] = 0;
            $data['Review']['cc_infection_uti_flag'] = 0;
            $data['Review']['cc_infection_endocarditis_flag'] = 0;
            $data['Review']['cc_infection_osteomyelitis_flag'] = 0;
            $data['Review']['cc_infection_meningitis_flag'] = 0;
            $data['Review']['cc_infection_cellulitis_flag'] = 0;
            $data['Review']['cc_infection_covid_flag'] = 0;
            $data['Review']['cc_infection_other_flag'] = 0;
        }

        $r = $data['Review'];

        if ($r['cc_infection_other_flag'] === '1') {
            if (!$r['cc_infection_other']) {
                return array(false, self::MISSING_INFECTION_OTHER);
            }
        } else {
            unset($data['Review']['cc_infection_other']);
        }

        $r = $data['Review'];

        if ($r['cc_ivdrug_flag'] === '1') {
            if (!$r['cc_ivdrug_use']) {
                return array(false, self::MISSING_IVDRUG_USE);
            }
        } else {
            unset($data['Review']['cc_ivdrug_use']);
        }
            
        $r = $data['Review'];

        if ($r['cc_other_flag'] === '1') {
            if (!$r['cc_other']) {
                return array(false, self::MISSING_CC_OTHER);
            }
        } else {
            unset($data['Review']['cc_other']);
        }
            
        $r = $data['Review'];

        if (isset($r['management_info']) && $r['management_info']) {
            if (!$r['management_at'] || !$r['management_hosp'] || 
                !$r['management_vcf'] || !$r['management_tt'] ||
                !$r['management_thrombectomy'] || !$r['management_managedas'])
            {
                return array(false, self::MISSING_MANAGEMENT_DATA);
            }
        } else {
            unset($data['Review']['management_at']);
            unset($data['Review']['management_hosp']);
            unset($data['Review']['management_vcf']);
            unset($data['Review']['management_tt']);
            unset($data['Review']['management_thrombectomy']);
            unset($data['Review']['management_managedas']);
        }
    ...
    /**
     * Can a particular user prescrub a particular event?
     * @param event The event
     * @param user the user
     */
    function canBePrescrubbed($event, $user) {
        return $this->canBePerformed('prescrub', $event) &&
               $this->canPerformAction('prescrub', $user, $event);
    }
    ...
    /** Message to return for bad screenAccept values for prescrubbing */
    const BAD_PRESCRUB_ACCEPT = 'You must either Accept or Reject';
     ...
     * @return status array:  (success => true/false; message => ...)
     */
    function prescrub($event, $dataEvent, $authUserId) {
        if (empty($dataEvent['prescrubAccept']) ||
            !in_array($dataEvent['prescrubAccept'], 
                      array(self::ACCEPT, self::REJECT))) 
        {
            return array('success' => false, 
                         'message' => self::BAD_PRESCRUB_ACCEPT);
        } else {
            $prescrubAccept = $dataEvent['prescrubAccept'];
            $event['Event']['prescrub_date'] = date('Y-m-d');
            $event['Event']['prescrubber_id'] = $authUserId;
            $fieldList = array('prescrub_date', 'prescrubber_id', 'status');

            if ($prescrubAccept == self::ACCEPT) {
                $event['Event']['status'] = self::PRESCRUBBED;
            } else { //if ($prescrubAccept == self::REJECT)
                $event['Event']['status'] = self::PSREJECTED;
            }

            $this->id = $event['Event']['id'];
            $this->save($event, true, $fieldList);
            $acceptMessage = $prescrubAccept . 'ed';
            $msg = PROJECT_NAME == 'MI' ? 
                     'Prescrubbed event MI ' . ($event['Event']['id'] + 1000) .
                     " ({$acceptMessage})" :
                     'Prescrubbed event ' . $event['Event']['id'] .
                     " ({$acceptMessage})";
    ...
    function getDownloadInfo($fileName, $suffix, $downloadPrefix, $id) {
        ...
        return array('sourcefile' => $this->pathPrefix() . "$fileName.$suffix",
                     'destfile' => "${downloadPrefix}_" . PROJECT_NAME .
                                   "_$id" . ".$suffix",
